      ...
      _.each(rule.declarations, function(declaration, declarationIndex) {                                            // 105
        ...
        while (parts = cssUrlRegex.exec(value)) {                                                                    // 115
          ...
          if (isRelative(resource.path) && resource.protocol === null) {                                             // 123
            ...
            // path.join() shouldn't be used on url paths. Fix up any slashes.                                       // 125
            if (path.sep !== '/') {                                                                                  // 126
              absolutePath = absolutePath.split(path.sep).join('/');                                                 // 127
            }                                                                                                        // 128
            newCssUrl = "url(" + quotes + absolutePath + quotes + ")";                                               // 129
            value = value.replace(oldCssUrl, newCssUrl);                                                             // 130
          }                                                                                                          // 131
        }                                                                                                            // 132
                                                                                                                     // 133
        declaration.value = value;                                                                                   // 134
      });                                                                                                            // 135
    });                                                                                                              // 136
  }                                                                                                                  // 137
};                                                                                                                   // 138
